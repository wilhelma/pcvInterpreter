...
public:
	...
	/// @brief Constructor.
	/// @details Initializes the `Query_` pointer and make the first step.
	...
	/// is over, the iterator is set to `end()` and returned. If errors
	/// occurr, an exception is thrown.
...
private:
	/// Pointer to QueryResult, which manages the query result.
...
const thread_t SQLStatementIterator<const thread_t>::operator*() const {
    ...
    NUM_CYCLES   num_cycles(Query_->get<NUM_CYCLES>(3));
	...
	return thread_t(id, start_time, end_time, num_cycles, create_instr_id, join_instr_id, parent_thread_id, process_id);
}
	...
	switch(Query_->step()) {
		...
		case SQLITE_DONE:
			*this = end();
			break;
		default:
			std::cerr << "Iteratind DB failed" << std::endl;
