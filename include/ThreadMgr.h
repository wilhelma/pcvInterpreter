...
public:
    ...
    /// @brief Get a pointer to the `ShadowThread` associated with `thread_id`.
    ...
    const std::shared_ptr<const ShadowThread> getThread(const TRD_ID& thread_id) noexcept;

    /// @brief Remove the entry corresponding to `thread_id` from the map.
    /// @throw ThreadManagerException If the queried thread ID is not in the map.
    /// @throw ThreadManagerException If the pointer corresponding to the thread ID is still in use.
    void threadJoined(const TRD_ID& thread_id);
