...
void CallSiteSpanProfile::add_in_callsite_span_profile_entries
			  (const std::shared_ptr<call_site_span_profile_t> profile_to_add) {
	prof->span += profile_to_add->span;
	prof->lock_span += profile_to_add->lock_span;
}

void CallSiteSpanProfile::init_callsite_span_profile(CALLSITE call_site, 
													 TIME span,
													 TIME lock_span) {
	prof->call_site = call_site;
	prof->span = span;
	prof->lock_span = lock_span;
}
